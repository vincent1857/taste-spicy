...
public class BusinessException extends RuntimeException implements BizException {
    ...
    public BusinessException(BusinessErrorCodeEnum error) {
        super(error.detail().getMessage());
        this.detail = error.detail().toErrorCode();
    }

    public BusinessException(BusinessErrorCodeEnum error, Throwable cause) {
        super(error.detail().getMessage(), cause);
        this.detail = error.detail().toErrorCode();
    }

    public BusinessException(BusinessErrorCodeEnum error, Serializable... args) {
        super(error.detail().toErrorCode(args).getMessage());
        this.detail = error.detail().toErrorCode(args);
    }

    public BusinessException(BusinessErrorCodeEnum error, Throwable cause, Serializable... args) {
