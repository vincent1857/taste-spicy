 ...
 */
public class SubjectException extends RuntimeException implements BizException {
    ...
    public SubjectException(ErrorCodeEnum error) {
        super(error.detail().getMessage());
        this.detail = error.detail().toErrorCode();
    }

    public SubjectException(ErrorCodeEnum error, Throwable cause) {
        super(error.detail().getMessage(), cause);
        this.detail = error.detail().toErrorCode();
    }

    public SubjectException(ErrorCodeEnum error, Serializable... args) {
        super(error.detail().toErrorCode(args).getMessage());
        this.detail = error.detail().toErrorCode(args);
    }

    public SubjectException(ErrorCodeEnum error, Throwable cause, Serializable... args) {
